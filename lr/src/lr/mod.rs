...
fn build_priority_queue(
    ...
    let mut edges = Vec::with_capacity((n - 1) * (n - 1));
    ...
    for i in 1..n {
        for j in 1..n {
            ...
            let cost = if forbidden_arcs.contains(&(i, j)) || forbidden_arcs.contains(&(j, i)) {
                f64::INFINITY
            } else {
                instance.distance(i, j) as f64 - lambdas[i] - lambdas[j]
            };
